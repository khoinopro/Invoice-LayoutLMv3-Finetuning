# Inference script for fine-tuned LayoutLMv3
...
model_weights = os.path.join(SRC_DIR, "model.bin")                # trained weights (best val loss)
model_base    = os.path.join(PROJECT_ROOT, "inputs", "layoutlmv3Microsoft")
model_path    = model_base if os.path.exists(model_base) else "microsoft/layoutlmv3-base"
config_path   = os.path.join(PROJECT_ROOT, "inputs", "label_config.json")

# *** Change this to the invoice image you want to run inference on ***
IMAGE_PATH = os.path.join(PROJECT_ROOT, "images", "invoice_CPC000692108_page0.png")
...
# ---------------------------------------------------------------------------
model = ModelModule(num_labels)
model.eval()

if os.path.exists(model_weights):
    model.load_state_dict(torch.load(model_weights, map_location="cpu"))
    print(f"[OK] Loaded weights from: {model_weights}")
else:
    print(f"[WARN] model.bin not found at {model_weights}. Using untrained weights.")
...
if __name__ == "__main__":
    ...
    OUTPUT_ROOT = os.path.join(PROJECT_ROOT, "inference_annotations")
    
    if not os.path.exists(IMAGES_DIR):
        print(f"[Error] Images folder not found at: {IMAGES_DIR}")
        exit(1)

    print(f"--- Starting Batch Inference from {IMAGES_DIR} ---")
    ...
    print(f"\n--- Batch Inference Complete! Results stored in {OUTPUT_ROOT} ---")